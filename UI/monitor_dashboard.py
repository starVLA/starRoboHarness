...
DEFAULT_ROOT = Path(os.environ.get("UNITY_POLICY_RUN_ROOT", "."))
DEFAULT_ASSET_DIR = Path(
    os.environ.get("UNITY_POLICY_DASHBOARD_DIR", Path(__file__).resolve().parent)
)
...
def aggregate_usage(root: Path):
    ...
    total["uncached_input_tokens"] = max(0, total["input_tokens"] - total["cached_input_tokens"])
...
def cost_summary(root: Path, history_roots):
    ...
    combined["standard_estimate_usd"] = (
        current["standard_estimate_usd"] + retained["standard_estimate_usd"]
    )
...
def media_info(root: Path, case_ids=None, media_prefix="/media"):
    ...
    if case_ids is not None:
        videos = [
            path
            for path in videos
            if len(path.parts) > len(root.parts) + 1 and path.parts[len(root.parts) + 1] in case_ids
        ]
    if videos:
...
def media_info_across(roots, case_ids, preferred_case=None):
    ...
    for index, root in enumerate(roots):
        ...
        paths = [
            path
            for path in paths
            if len(path.parts) > len(root.parts) + 1 and path.parts[len(root.parts) + 1] in selected
        ]
        if paths:
...
def collect(root: Path, history_roots=HISTORY_ROOTS):
    ...
    supervisor_candidates = [root.parent / f"{root.name}-supervisor"]
    if root.name.endswith("-results"):
        supervisor_candidates.append(root.parent / f"{root.name[:-8]}-supervisor")
    supervisor_dir = next(
        (path for path in supervisor_candidates if path.exists()), supervisor_candidates[0]
    )
    supervisor_exit = read_json(supervisor_dir / "exit.json", {}) or {}
    ...
    if invalid_attempts and panel and not campaign.get("finished"):
        ...
        run_note = (
            "已出现基础设施中断；冻结调度器会让当前在途 episode 收尾后停止，不会自动完成剩余计划。"
        )
    ...
    elif supervisor_exit.get("finished"):
        state = "STOPPED / INCOMPLETE"
        run_note = "监督器已结束；未形成完整有效结果，历史 evidence 已保留。"
    ...
    log_lines = tail(supervisor_dir / "campaign.log")
...
def collect_combined(root: Path, additional_roots=(), history_roots=HISTORY_ROOTS):
    ...
    for additional_root in additional_roots:
        ...
        status["finished"] = status.get("finished", False) and extra_status.get("finished", False)
...
def write_state(asset_dir: Path, root: Path, history_roots=HISTORY_ROOTS, additional_roots=()):
...
def main():
    ...
    thread.start()
    media_roots = evidence_roots(args.root, read_json(args.root / "outcomes.json", []) or [])
