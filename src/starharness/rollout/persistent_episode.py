...
The host exposes an advisory recovery watchdog. After three consecutive corrections,
do not repeat the same correction target: use the fresh observation to choose a
distinct grasp/replan. Distinct corrections with measured physical progress remain
valid; the advisory threshold never ends an episode. RoboDojo native termination is
authoritative. Avoid unchanged no-progress micro-correction loops.
"""
...
def run_persistent(*, root, method, case, config, connection, sim_port, runtime_identity):
    ...
    worker = PersistentAgent(
        ...
        no_action_tokens=config.get("no_action_tokens", 250000), task=case["runtime_task"],
        app_server_timeout=config.get("app_server_timeout", 180),
        reasoner_restart_attempts=config.get("reasoner_restart_attempts", 0))
    rollout = None
    try:
        ...
        kwargs = dict(provider=provider, sim_port=sim_port, seed=case["layout_id"],
                      ...
                      max_consecutive_recoveries=config.get("max_consecutive_recoveries", 8))
