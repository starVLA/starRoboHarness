    ...
    def infer(
        ...
        if self.base_url is not None:
            command.extend(
                [
                    "-c",
                    'model_provider="starroboharness"',
                    "-c",
                    'model_providers.starroboharness.name="StarRoboHarness"',
                    "-c",
                    f"model_providers.starroboharness.base_url={json.dumps(self.base_url)}",
                    "-c",
                    'model_providers.starroboharness.wire_api="responses"',
                    "-c",
                    'model_providers.starroboharness.env_key="STARROBOHARNESS_API_KEY"',
                    "-c",
                    "model_providers.starroboharness.request_max_retries=0",
                    "-c",
                    "model_providers.starroboharness.stream_max_retries=0",
                ]
            )
        for path in paths:
        ...
        if self.api_key_file is not None:
            ...
            environment["STARROBOHARNESS_API_KEY"] = key
        metadata: dict[str, Any] = {
            "schema": "starroboharness.codex_call.v1",
