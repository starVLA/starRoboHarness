...
class Cluster:
    ...
    def read(self, pod, path, *, timeout=30, attempts=1):
        """Read a remote artifact, retrying transient kubectl transport stalls."""
        last_error = None
        for attempt in range(attempts):
            try:
                result = subprocess.run(
                    self.command(pod, "cat " + shlex.quote(str(path))),
                    capture_output=True,
                    text=True,
                    timeout=timeout,
                )
    ...
    def start(self, pod, spec, output):
        ...
        command = (
            ...
            f"{shlex.quote(cfg['remote_python'])} -m starroboharness.rollout.service"
        )
    ...
    def _forward_locked(self, pod, local, remote, path):
        ...
        for attempt in range(1, attempts + 1):
            ...
            log = path.open("a")
            log.write(f"\n[star-robo-harness port-forward attempt {attempt}/{attempts}]\n")
            log.flush()
            process = subprocess.Popen(
...
def episode(cluster, panel, case, method, slot, root):
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            try:
                sim_log = cluster.read(cfg["sim_pod"], remote / "sim-service/service.log")
                ...
                policy_ready = not learned or "server listening" in cluster.read(
                    cfg["policy_pod"], remote / "policy/service.log"
                )
